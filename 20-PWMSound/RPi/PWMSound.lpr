...
uses
  ...
  BCM2708,      {Include the BCM2708 and BCM2835 units for access to the PWM device}
  BCM2835;      {and PWM register values and constants.}
...
begin
 ...
 if not BCM2708PWMClockEnabled(PWM) then
  begin
   ...
   {Set Source}   
   PLongWord(BCM2835_CM_REGS_BASE + BCM2835_CM_PWMCTL)^:=BCM2835_CM_PASSWORD or BCM2835_CM_CTL_SRC_PLLD;
   {Delay}
   MicrosecondDelay(10);
   ...
   if PWM_LOG_ENABLED then PWMLogDebug(PWM,'PWM Sound:  PWMCTL=' + IntToHex(PLongWord(BCM2835_CM_REGS_BASE + BCM2835_CM_PWMCTL)^,8));
   if PWM_LOG_ENABLED then PWMLogDebug(PWM,'PWM Sound:  PWMDIV=' + IntToHex(PLongWord(BCM2835_CM_REGS_BASE + BCM2835_CM_PWMDIV)^,8));
   {$ENDIF}
...
begin
 ...
 case PBCM2708PWMDevice(PWM).Channel of
  0:begin
    ...
    PBCM2835PWMRegisters(PBCM2708PWMDevice(PWM).Address).CTL:=PBCM2835PWMRegisters(PBCM2708PWMDevice(PWM).Address).CTL or BCM2835_PWM_CTL_PWEN1 or BCM2835_PWM_CTL_USEF1 or BCM2835_PWM_CTL_CLRF1;
   end;
  1:begin
    {PWM1 (PWM Channel 2)}
    {Enable PWEN, USEF and CLRF}
    PBCM2835PWMRegisters(PBCM2708PWMDevice(PWM).Address).CTL:=PBCM2835PWMRegisters(PBCM2708PWMDevice(PWM).Address).CTL or BCM2835_PWM_CTL_PWEN2 or BCM2835_PWM_CTL_USEF2 or BCM2835_PWM_CTL_CLRF1;
   end;
  else
   begin
    Exit;
 ...
 {Clear Status}
 PBCM2835PWMRegisters(PBCM2708PWMDevice(PWM).Address).STA:=LongWord(-1);
 
 {$IF DEFINED(BCM2708_DEBUG) or DEFINED(PWM_DEBUG)}
 if PWM_LOG_ENABLED then PWMLogDebug(PWM,'PWM Sound:  CTL=' + IntToHex(PBCM2835PWMRegisters(PBCM2708PWMDevice(PWM).Address).CTL,8));
 if PWM_LOG_ENABLED then PWMLogDebug(PWM,'PWM Sound:  STA=' + IntToHex(PBCM2835PWMRegisters(PBCM2708PWMDevice(PWM).Address).STA,8));
 if PWM_LOG_ENABLED then PWMLogDebug(PWM,'PWM Sound:  DMAC=' + IntToHex(PBCM2835PWMRegisters(PBCM2708PWMDevice(PWM).Address).DMAC,8));
 if PWM_LOG_ENABLED then PWMLogDebug(PWM,'PWM Sound:  RNG1=' + IntToHex(PBCM2835PWMRegisters(PBCM2708PWMDevice(PWM).Address).RNG1,8));
 if PWM_LOG_ENABLED then PWMLogDebug(PWM,'PWM Sound:  DAT1=' + IntToHex(PBCM2835PWMRegisters(PBCM2708PWMDevice(PWM).Address).DAT1,8));
 if PWM_LOG_ENABLED then PWMLogDebug(PWM,'PWM Sound:  RNG2=' + IntToHex(PBCM2835PWMRegisters(PBCM2708PWMDevice(PWM).Address).RNG2,8));
 if PWM_LOG_ENABLED then PWMLogDebug(PWM,'PWM Sound:  DAT2=' + IntToHex(PBCM2835PWMRegisters(PBCM2708PWMDevice(PWM).Address).DAT2,8));
 {$ENDIF}
...
begin
 ...
 {$IF DEFINED(BCM2708_DEBUG) or DEFINED(PWM_DEBUG)}
 if PWM_LOG_ENABLED then PWMLogDebug(PWM,'PWM Sound:  Scaler=' + IntToStr(PBCM2708PWMDevice(PWM).Scaler));
 {$ENDIF}
...
begin 
 ...
 try
  ...
  {Enable DMA}
  PBCM2835PWMRegisters(PBCM2708PWMDevice(PWM).Address).DMAC:=PBCM2835PWMRegisters(PBCM2708PWMDevice(PWM).Address).DMAC or BCM2835_PWM_DMAC_ENAB;
