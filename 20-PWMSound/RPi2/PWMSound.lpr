...
uses
  ...
  BCM2709,      {Include the BCM2709 and BCM2836 units for access to the PWM device}
  BCM2836;      {and PWM register values and constants.}
...
begin
 ...
 if not BCM2709PWMClockEnabled(PWM) then
  begin
   ...
   {Set Source}   
   PLongWord(BCM2836_CM_REGS_BASE + BCM2836_CM_PWMCTL)^:=BCM2836_CM_PASSWORD or BCM2836_CM_CTL_SRC_PLLD;
   {Delay}
   MicrosecondDelay(10);
   ...
   if PWM_LOG_ENABLED then PWMLogDebug(PWM,'PWM Sound:  PWMCTL=' + IntToHex(PLongWord(BCM2836_CM_REGS_BASE + BCM2836_CM_PWMCTL)^,8));
   if PWM_LOG_ENABLED then PWMLogDebug(PWM,'PWM Sound:  PWMDIV=' + IntToHex(PLongWord(BCM2836_CM_REGS_BASE + BCM2836_CM_PWMDIV)^,8));
   {$ENDIF}
...
begin
 ...
 case PBCM2709PWMDevice(PWM).Channel of
  0:begin
    ...
    PBCM2836PWMRegisters(PBCM2709PWMDevice(PWM).Address).CTL:=PBCM2836PWMRegisters(PBCM2709PWMDevice(PWM).Address).CTL or BCM2836_PWM_CTL_PWEN1 or BCM2836_PWM_CTL_USEF1 or BCM2836_PWM_CTL_CLRF1;
   end;
  1:begin
    {PWM1 (PWM Channel 2)}
    {Enable PWEN, USEF and CLRF}
    PBCM2836PWMRegisters(PBCM2709PWMDevice(PWM).Address).CTL:=PBCM2836PWMRegisters(PBCM2709PWMDevice(PWM).Address).CTL or BCM2836_PWM_CTL_PWEN2 or BCM2836_PWM_CTL_USEF2 or BCM2836_PWM_CTL_CLRF1;
   end;
  else
   begin
    Exit;
 ...
 {Clear Status}
 PBCM2836PWMRegisters(PBCM2709PWMDevice(PWM).Address).STA:=LongWord(-1);
 
 {$IF DEFINED(BCM2709_DEBUG) or DEFINED(PWM_DEBUG)}
 if PWM_LOG_ENABLED then PWMLogDebug(PWM,'PWM Sound:  CTL=' + IntToHex(PBCM2836PWMRegisters(PBCM2709PWMDevice(PWM).Address).CTL,8));
 if PWM_LOG_ENABLED then PWMLogDebug(PWM,'PWM Sound:  STA=' + IntToHex(PBCM2836PWMRegisters(PBCM2709PWMDevice(PWM).Address).STA,8));
 if PWM_LOG_ENABLED then PWMLogDebug(PWM,'PWM Sound:  DMAC=' + IntToHex(PBCM2836PWMRegisters(PBCM2709PWMDevice(PWM).Address).DMAC,8));
 if PWM_LOG_ENABLED then PWMLogDebug(PWM,'PWM Sound:  RNG1=' + IntToHex(PBCM2836PWMRegisters(PBCM2709PWMDevice(PWM).Address).RNG1,8));
 if PWM_LOG_ENABLED then PWMLogDebug(PWM,'PWM Sound:  DAT1=' + IntToHex(PBCM2836PWMRegisters(PBCM2709PWMDevice(PWM).Address).DAT1,8));
 if PWM_LOG_ENABLED then PWMLogDebug(PWM,'PWM Sound:  RNG2=' + IntToHex(PBCM2836PWMRegisters(PBCM2709PWMDevice(PWM).Address).RNG2,8));
 if PWM_LOG_ENABLED then PWMLogDebug(PWM,'PWM Sound:  DAT2=' + IntToHex(PBCM2836PWMRegisters(PBCM2709PWMDevice(PWM).Address).DAT2,8));
 {$ENDIF}
...
begin
 ...
 {$IF DEFINED(BCM2709_DEBUG) or DEFINED(PWM_DEBUG)}
 if PWM_LOG_ENABLED then PWMLogDebug(PWM,'PWM Sound:  Scaler=' + IntToStr(PBCM2709PWMDevice(PWM).Scaler));
 {$ENDIF}
...
begin 
 ...
 try
  ...
  DMAData.Dest:=PBCM2709PWMDevice(PWM).Address + BCM2836_PWM_FIF1;
  ...
  {Enable DMA}
  PBCM2836PWMRegisters(PBCM2709PWMDevice(PWM).Address).DMAC:=PBCM2836PWMRegisters(PBCM2709PWMDevice(PWM).Address).DMAC or BCM2836_PWM_DMAC_ENAB;
...
begin
 ...
 if (PWM0Device <> nil) and (PWM1Device <> nil) then
  begin
   ...
   {Setup PWM device 1}
   {Use exactly the same settings as PWM0 except the GPIO is 45}
   PWMDeviceSetGPIO(PWM1Device,GPIO_PIN_45);
