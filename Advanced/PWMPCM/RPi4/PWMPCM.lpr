...
const
 PWMPCM_PWM_OSC_CLOCK = 54000000;   
 PWMPCM_PWM_PLLD_CLOCK = 750000000;
...
begin
 ...
 if not BCM2711PWM0ClockEnabled(PWM) then
  begin
   ...
   if PWM_LOG_ENABLED then PWMLogDebug(PWM,'PWMPCM:  PWMCTL=' + IntToHex(PLongWord(BCM2838_CM_REGS_BASE + BCM2838_CM_PWMCTL)^,8));
   if PWM_LOG_ENABLED then PWMLogDebug(PWM,'PWMPCM:  PWMDIV=' + IntToHex(PLongWord(BCM2838_CM_REGS_BASE + BCM2838_CM_PWMDIV)^,8));
   {$ENDIF}
   ...
   begin
    ...
    while ((Status and BCM2838_PWM_STA_FULL1) = 0) and (Count < Samples) do
     begin
      ...
      if (Status and BCM2838_PWM_STA_EMPT1) <> 0 then
       begin
        Inc(EmptyCount);
       end; 
    ...
    {Check for FIFO full}
    if (Status and BCM2838_PWM_STA_FULL1) <> 0 then
     begin
      Inc(FullCount);
      MicrosecondDelay(10);
     end; 
...
begin
 ...
 {First locate the PWM device
  ...
  For this example we only need PWM2, you can setup both channels to play
